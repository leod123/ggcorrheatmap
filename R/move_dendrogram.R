...
#' @param dend_pos Logical specifying dendrogram position. TRUE is left of the heatmap if row dendrogram, top of heatmap if column dendrogram
...
#' @param annot_pos Logical specifying annotation position, analogous to `dend_pos`
#' @param annot_coord Numeric vector of annotation coordinates (x coordinates for row annotations, y for column annotations)
...
#'
move_dendrogram <- function(cor_long, dend_seg, dend_dim = c("rows", "cols"), dend_pos,
                            annot_df, annot, annot_pos, annot_coord, annot_size) {
  ...
  if (dend_dim[1] == "rows") {
    ...
    xmove <- ifelse(
      ...
      ifelse(    # Annotation left of rows or not
        is.data.frame(annot_df) & annot & annot_pos,
        min(annot_coord) - 0.5 * annot_size - max(c(dend_seg$x, dend_seg$xend)),
        0.5 - max(c(dend_seg$x, dend_seg$xend))
      ),
      ifelse(    # Annotation right of rows or not
        is.data.frame(annot_df) & annot & !annot_pos,
        max(annot_coord) + 0.5 * annot_size - min(c(dend_seg$x, dend_seg$xend)),
    ...
    ymove <- ifelse(
      ...
      ifelse(    # Annotation above heatmap
        is.data.frame(annot_df) & annot & annot_pos,
        max(annot_coord) + 0.5 * annot_size - min(c(dend_seg$y, dend_seg$yend)),
        length(unique(cor_long$col)) + 0.5 - min(c(dend_seg$y, dend_seg$yend))
      ),
      ifelse(    # Annotation below heatmap
        is.data.frame(annot_df) & annot & !annot_pos,
        min(annot_coord) - 0.5 * annot_size - max(c(dend_seg$y, dend_seg$yend)),

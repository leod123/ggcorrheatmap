...
test_cor <- function(x, y = NULL, full_plt = T, include_diag = F,
  ...
  if (is.null(y) | identical(x, y)) {
    ...
    all_pairs <- if (full_plt) {
      shape_mat_long(matrix(nrow = ncol(x), ncol = ncol(x), dimnames = list(colnames(x), colnames(x))))
    } else {
      test_pairs
    }
    ...
    if (!full_plt & !include_diag) {
      # If diagonal is not drawn, return only the non-diagonal values
      return(test_pairs)

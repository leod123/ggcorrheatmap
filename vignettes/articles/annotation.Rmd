...
gghm(cor(mtcars), annot_rows_df = annot, annot_cols_df = annot,
     ...
     annot_cols_label_side = "right", legend_position = "left",
     plot_margin = c(10, 50, 30, 10))
gghm(cor(mtcars), annot_rows_df = annot, annot_cols_df = annot,
     annot_rows_side = "left", annot_cols_side = "top",
     plot_margin = c(20, 10, 10, 50),
...
If the annotation labels are too long and are cut off, the plot margins can be adjusted with the `plot_margin` argument. Alternatively, the figure height can be increased, as the heatmap is set to use a fixed aspect ratio (`ggplot2::coord_fixed`).
...
As can be seen in the plots above, the legends may be cut off if there are many annotations. Fitting the legends can be tricky and may require increasing the figure size. Alternatively a package like `cowplot` can be used to stitch together a legendless version of the plot with a grid of only the legends.

If the annotation data frame contains names not in the heatmap, new rows/columns will be made just to fit the annotation, although this usage is unintended.

```{r}
row_annot <- data.frame(.names = c(colnames(mtcars), letters[1:10]),
                        a = rnorm(ncol(mtcars) + 10))

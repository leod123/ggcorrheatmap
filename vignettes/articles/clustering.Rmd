...
```{r, fig.show='hold', out.width='50%', fig.alt='Two mtcars correlation heatmaps. The first one has the dendrograms on the top and left. The second plot has only drawn the bottom left triangle of the matrix and there is a dendrogram on the left side.'}
...
```{r}
gghm(scale(mtcars[1:5, ]), cluster_rows = TRUE)
gghm(scale(mtcars[1:5, ]), cluster_rows = TRUE,

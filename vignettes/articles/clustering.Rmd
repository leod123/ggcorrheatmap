...
The positions of the dendrograms can be changed with the `dend_rows_side` ('left' or 'right') and `dend_cols_side` ('top' or 'bottom') arguments. For triangular layouts the dendrograms are placed on the non-empty sides of the heatmap.
...
Leaves can be reordered using functions like `ladderize` or `rotate`, though it is not recommended to do for only one dimension if the matrix is square as the diagonal may end up in strange places. In such cases, triangular layouts are not supported.

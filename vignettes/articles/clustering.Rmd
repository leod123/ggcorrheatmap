...
Heatmap rows and columns are clustered if the `cluster_rows` and `cluster_cols` arguments are set to `TRUE`. Distance measure and clustering method can be specified with the `cluster_distance` and `cluster_method` arguments, passed on to the `dist` and `hclust` functions, respectively.
...
Using the `..._extend` arguments also makes it possible to display the nodes of the dendrograms.

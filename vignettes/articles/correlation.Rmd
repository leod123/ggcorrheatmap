...
`ggcorrhm()` sets the fill scale to use `ggplot2::scale_fill_gradient2()` with limits set to [-1, 1]. Unique to `ggcorrhm()` are the arguments `high`, `mid`, `low`, `limits`, and `bins` to control the legend colours. `bins` creates a binned scale (using `ggplot2::scale_fill_steps2()` instead). 
...
ggcorrhm(mtcars, high = "green", low = "magenta", mid = "yellow", bins = 6)

```

If complete control is needed, the `fill_scale` and `col_scale` arguments can be used to overwrite the default scale.
...
plt1 <- ggcorrhm(mtcars, fill_scale =
                   # scale_fill_distiller is convenient for using the
                   # Brewer palettes for continuous data
                   scale_fill_distiller(palette = "RdYlGn", limits = c(-1, 1)))

plt2 <- ggcorrhm(mtcars, fill_scale =
                   # scale_fill_fermenter can make binned Brewer palettes
                   # for continuous data
                   scale_fill_fermenter(palette = "PuOr", limits = c(-1, 1),
...
Correlation heatmaps are sometimes plotted with circles. Passing a value from 1 to 25 to the `mode` argument allows for different cell shapes. Shapes 21-25 support filling and use fill scales (the `fill_scale` argument). 1-20 are not filled and use colour scales (the `col_scale` argument). The shape size is set to vary between the values in the `size_range` argument (not found in `gghm()`), scaling with the absolute value of the correlation. `size_scale` can be provided to overwrite this behaviour. The size legend is hidden by default when using `ggcorrhm()` as it would only show the absolute values of the correlations (but the legend can be shown with the `show_legend` argument if necessary).
...
In a mixed layout it is possible to display p-values in only half of the plot by passing a vector to the `p_values` argument. If `cell_labels` is `TRUE` (so that correlation values are written in the cells), the symbols from `p_thresholds` are added at the end of the correlation values. 

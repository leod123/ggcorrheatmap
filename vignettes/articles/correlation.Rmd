...
The legend can be moved around using the `legend_position` argument (specifying 'top', 'bottom', 'left', or 'right'). If a numeric vector of length two is given, the legend is placed at those coordinates within the drawing area (scaled to range between 0 and 1).

```{r legend position, fig.width=5, fig.height=4, fig.show='hold', out.width='50%'}
ggcorrhm(mtcars, legend_position = "top")
ggcorrhm(mtcars, layout = "br", legend_position = c(0.2, 0.7))
```
...
P-values can be calculated by setting the `p_calc` argument to `TRUE`. The values can be adjusted for multiple testing by passing a string specifying the adjustment method to `p_adj`. If the correlation matrix is symmetric, the diagonal is excluded and only half of the off-diagonal values are used for the adjustment. This alone only means that the p-values will be calculated (which will be a bit slower than if `p_calc` is `FALSE`). The calculated p-values (nominal and adjusted) can be returned in the output by setting `return_data` to `TRUE`. To mark cells, set `p_thr` to a value between 0 and 1 and adjusted p-values below the threshold will be marked by asterisks (if `label_cor` is `TRUE` the asterisk is added to the correlation value). 

```{r, fig.show='hold', out.width='50%'}
ggcorrhm(mtcars, p_calc = TRUE, p_thr = 0.05, cell_label_size = 6)
ggcorrhm(mtcars, p_calc = TRUE, p_thr = 0.05, label_cor = TRUE)
```
...
```{r}
p <- ggcorrhm(mtcars, p_calc = TRUE, p_adj = "bonferroni", return_data = TRUE,
...
p$plot_data <- mutate(p$plot_data,
                      ...
                      p_lab = case_when(p_adj < 0.001 ~ "***", p_adj < 0.01 ~ "**", p_adj < 0.05 ~ "*", T ~ ""))
ggcorrhm(mtcars, border_lty = p$plot_data$p_lty, border_lwd = p$plot_data$p_lwd,
         layout = "br", include_diag = TRUE) +
  geom_text(aes(label = p_lab), data = p$plot_data %>%

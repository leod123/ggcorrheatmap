...
`ggcorrhm()` sets the fill scale to use `ggplot2::scale_fill_gradient2()` with limits set to [-1, 1]. Unique to `ggcorrhm()` are the arguments `high`, `mid`, `low`, `limits`, and `bins` to control the legend colours. `bins` creates a binned scale (with the specified number of bins, switches to using `ggplot2::scale_fill_steps2()` instead). If complete control is needed, the `fill_scale` and `col_scale` arguments can be used to overwrite the gradient scale.
...
```{r changing colour, fig.alt='A heatmap of the mtcars correlation matrix. The colour scale is changed to consist of 6 bins and the colours go from magenta to yellow to green.'}
...
```{r legend position, fig.width=10, fig.height=5, fig.show='hold', out.width='50%', fig.alt='Two mtcars correlation heatmaps. In the first one the legend has been moved above the heatmap and is horizontal. In the second the bottom right triangle is drawn and the legend is placed in the empty space where the top left triangle would be drawn.'}
plt1 <- ggcorrhm(mtcars) + theme(legend.position = "top", legend.title = element_text(vjust = 0.8))
...
The `cell_labels` argument labels cells with their values or some user-supplied values, explained more in the [heatmap article](heatmap.html#cell-labels). If `cell_labels` is a matrix/data frame it should be in the shape of the matrix being plotted, which for `ggcorrhm` is the correlation matrix. To keep in mind for triangular layouts is that the top left or bottom right triangles will display the bottom left or top right triangles with the y axis order flipped, and this is reflected in the cell labels too.
...
plt_list <- lapply(c("tl", "tr", "bl", "br"), function(lt) {
  ...
  show_legend <- if (lt == "br") c(fill = TRUE, colour = FALSE) else FALSE
  ggcorrhm(mtcars, layout = lt, show_legend = show_legend,
           cell_labels = lab_mat, names_diag = FALSE) +
    theme(legend.position = "inside", legend.position.inside = c(0.025, 1.075))
})
...
In a mixed layout it is possible to display p-values in only half of the plot by passing a vector to the `p_values` argument. If `cell_labels` is `TRUE` (so that correlation values are written in the cells), the symbols from `p_thresholds` are added at the end of the correlation values. It is also possible to write out the p-values instead of correlation values by setting both `cell_labels` and `cell_label_p` to `TRUE`. This is also carried over to 'text' mode.

```{r, fig.width=15, fig.height=5, fig.alt='Three mtcars correlation heatmaps, all with filled cells in the top left and labelled white cells in the bottom right. The first heatmap has correlation values written in the bottom right, with asterisks signifying the p-value. The second plot has labels in both triangles; correlation values in the top left and p-values with asterisks in the bottom right. The third plot is similar to the second, but the p-value labels are coloured with the same scale as the filled cells in the top left triangle (scaling with the correlation values).'}
# P-values only in the bottom right, mode 'none' for labels with empty background
plt1 <- ggcorrhm(mtcars, layout = c("tl", "br"), mode = c("hm", "none"), p_adjust = "bonferroni",
                 p_values = c(FALSE, TRUE), cell_labels = c(FALSE, TRUE))
# P-values and labels only in one half, correlation labels in the other
plt2 <- ggcorrhm(mtcars, layout = c("tl", "br"), mode = c("hm", "none"), p_adjust = "bonferroni",
                 p_values = c(FALSE, TRUE), cell_labels = TRUE,
                 cell_label_p = c(FALSE, TRUE))
# Same as previous but text mode
plt3 <- ggcorrhm(mtcars, layout = c("tl", "br"), mode = c("hm", "text"), p_adjust = "bonferroni",

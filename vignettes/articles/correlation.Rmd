...
`ggcorrhm()` sets the fill scale to use `ggplot2::scale_fill_gradient2()` with limits set to [-1, 1]. Unique to `ggcorrhm()` are the arguments `high`, `mid`, `low`, `limits`, and `bins` to control the legend colours. `bins` creates a binned scale (using `ggplot2::scale_fill_steps2()` instead). If complete control is needed, the `fill_scale` and `col_scale` arguments can be used to overwrite the gradient scale.

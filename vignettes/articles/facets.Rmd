...
Gaps can be introduced into heatmaps with the `facet_rows` and `facet_cols` arguments (as the names imply the gaps are made by adding facets to the plot). The simplest way to use these arguments is to provide numeric vectors with the indices of the gap positions (counting from the top for rows and left for columns).

```{r}
plt1 <- gghm(mtcars, legend_order = NA)

plt2 <- gghm(mtcars, legend_order = NA,
             facet_rows = c(10, 20), facet_cols = 6)

plt1 + plt2
```

Without facets, the heatmaps use `ggplot2::coord_fixed()` to make the cells square. As this does not work facets and free scales, the cells will stretch with the plot when there are facets.

When `facet_rows` and `facet_cols` are shorter than the number of rows or columns, respectively, they work as shown above. It is also possible to provide vectors of the same lengths as the corresponding dimensions. In this case they are treated as the facet memberships for the rows or columns and the facet names are displayed as well. `facet_rows_side` and `facet_cols_side` change the sides where the names are displayed.

```{r}
gghm(mtcars,
     # Two facets '123' and '456' for columns
     facet_cols = c(rep(123, 6), rep(456, 5)),
     # Three facets 'AAA', 'BB', and 'C' for rows
     facet_rows = c(rep("AAA", 10), rep("BB", 10), rep("C", 12)))
```

Rows or columns may end up in a different order compared to the input depending on the facetting (e.g. disp is in the second facet in the first plot below even though it's the third column in the input). Within each facet the rows/columns will be ordered as they were in the input as far as possible. The facets themselves are ordered by the order of appearance in the `facet_rows` and `facet_cols` vectors. If they are factor vectors the levels will decide the order.
...
plt1 <- gghm(mtcars, legend_order = NA,
             facet_rows = row_facets,
             facet_cols = col_facets)
...
plt2 <- gghm(mtcars, legend_order = NA,
             facet_rows = row_facets2,
             facet_cols = col_facets2)
...
gghm(mtcars, scale_data = "col",
     facet_rows = row_clust,
     facet_cols = col_clust)
```
...
# Just using indices
plt1 <- ggcorrhm(mtcars, facet_rows = 5, facet_cols = 5, legend_order = NA) +
...
# Two from the top
plt1 <- ggcorrhm(mtcars, facet_rows = 2, facet_cols = 2,
                 layout = "bl")
# Two from the bottom
plt2 <- ggcorrhm(mtcars, facet_rows = 2, facet_cols = 2,
                 layout = "br")
...
gghm(plt_dat,
     facet_rows = rep(letters[1:4], each = 5),
     facet_cols = rep(LETTERS[1:4], each = 5)) +
  theme(
...
If clustering is applied, `facet_rows` and `facet_cols` work differently and only take single numeric values. The number is given to `stats::cutree()` to divide the data into clusters that then define the facets of the plot. In this case the dendrograms cannot be shown.
...
ggcorrhm(mtcars, facet_rows = facet_mem, facet_cols = facet_mem,

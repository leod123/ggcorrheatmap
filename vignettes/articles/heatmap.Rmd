...
gghm(replicate(15, sample(letters[1:3], 15, TRUE)),
     fill_scale = scale_fill_brewer(palette = "Pastel2"), border_col = "thistle1", border_lwd = 1)
```

Cell borders are by default grey, but can be removed by setting `border_col` to NA (if `cell_shape` is non-numeric, explained more in [correlation heatmaps](correlation.html)) or `border_lwd` to 0 (if `cell_shape` is numeric).

```{r}
gghm(volcano, fill_scale = scale_fill_viridis_c(), border_col = NA, names_x = FALSE, names_y = FALSE)
```
...
```{r annotation}
row_annot <- data.frame(.names = rownames(mtcars), a = sample(letters[1:4], nrow(mtcars), T))

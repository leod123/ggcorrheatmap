...
If the matrix being plotted is square and has the same row- and column names, the axis labels are drawn on the diagonal. This behaviour can be changed using the `show_names_diag`, `show_names_x` and `show_names_y` arguments. Such a heatmap can also be plotted using triangular and mixed triangular layouts, explained more in the [correlation matrix](correlation.html) and [mixed layouts](mixed.html) articles.

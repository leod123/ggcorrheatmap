...
The colours of the heatmap can be adjusted by passing a string specifying a Brewer or Viridis scale or a `ggplot2::scale_fill_*` function call to the `colr_scale` argument. The limits can be changed by giving a vector to the `limits` argument.

```{r, fig.alt='A heatmap of the whole mtcars data using the viridis colour scale.'}
# Use the Viridis D (can also be "viridis") option
gghm(mtcars, colr_scale = "D")
```

The `bins` argument creates a binned scale (only for continuous data). If `bins` is a float number (class 'double'), `ggplot2` will try to put the breaks at nice numbers at the cost of changing the number of bins. If `bins` is an integer the exact number of bins will instead be attempted, but the breaks may end up at strange numbers.
...
plt1 <- gghm(mtcars, colr_scale = "A", bins = 6)
plt2 <- gghm(mtcars, colr_scale = "A", bins = 6L)
...
gghm(replicate(15, sample(letters[1:3], 15, TRUE)),
     colr_scale = "Pastel2", border_col = "thistle1",
     border_lwd = 1)
```

For `gghm()` fill, colour and size legends are shown by default. The legend order can be changed using the `legend_order` argument. `ggcorrhm()` tries to hide some legends by default. Legends and scales are covered more in the [legends article](legends.html).
...
plt1 <- gghm(cor(mtcars), colr_scale = "G",
...
plt2 <- gghm(cor(mtcars), colr_scale = "G",
...
NAs in the data can be removed using the `na_remove` argument or set to a specific colour using the `na_col` and `colr_scale` arguments. Setting `na_remove` to `TRUE` removes NAs from the plot, but they can still interfere with clustering if a whole row or column is all NAs.
...
na_plt1 <- gghm(df_na, legend_order = NA) + labs(title = "NAs default")
# Colour NAs using scale function
na_plt2 <- gghm(df_na, legend_order = NA, names_y = FALSE,
                na_col = "white") +
  labs(title = "Change NA colour")
# Remove NAs, removes the border as well
na_plt3 <- gghm(df_na, legend_order = NA, names_y = FALSE, na_remove = TRUE) +
...
plt1 <- gghm(plot_data, colr_scale = fill_scl, cell_labels = TRUE, cell_label_size = 5) +
  labs(title = "Using cell values")
plt2 <- gghm(plot_data, colr_scale = fill_scl, cell_labels = label_mat, cell_label_size = 5) +
...
plt1 <- gghm(plot_data, cell_labels = TRUE, colr_scale = fill_scl,
...
plt2 <- gghm(plot_data, cell_labels = label_mat, mode = "text",
             colr_scale = col_scl, cell_label_size = 8) +
...
plot_list <- gghm(scale(mtcars), return_data = TRUE, border_col = NA,
                  colr_scale = scale_fill_gradient2(low = "deepskyblue", mid = "white", high = "salmon"))

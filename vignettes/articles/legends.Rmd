...
As explained in some of the other articles, the colour scale argument `colr_scale` takes NULL (for default scales), a character (for Brewer and Viridis scales) and `ggplot2` scale objects. When passing scale objects, make sure the correct aesthetic is used (fill or colour/color depending on plotting mode).
...
plt1 <- gghm(volcano, colr_name = "Default",
             border_col = NA, names_x = FALSE, names_y = FALSE)

plt2 <- gghm(volcano,
             # Brewer Spectral palette
             colr_scale = "A", colr_name = "Character",
             border_col = NA, names_x = FALSE, names_y = FALSE)

plt3 <- gghm(volcano,
             # Custom scale
             colr_scale = scale_fill_gradient2(
...
This is also true for the annotation scale arguments `annot_rows_fill` and `annot_cols_fill`.
...
gghm(mtcars[1:10, 1:5],
     ...
     annot_rows_fill = row_annot_fill,
...
When `colr_scale` is a character, "rev_" or "_rev" can be added to the beginning or end, respectively, to reverse the scale. This also works for annotation scales.
...
gghm(volcano, names_x = FALSE, names_y = FALSE, border_col = NA,
     colr_scale = "rev_A", colr_name = "Reversed",
     annot_rows_df = row_annot,
     annot_rows_fill = row_annot_colr,
...
When the plot is using a mixed layout, `colr_scale` can also take a vector or list to apply different scales to the different triangles.
...
gghm(plt_dat, layout = c("tl", "br"),
     mode = c("hm", "text"), colr_scale = "A")
```

When using scale objects, arguments that change scales and legends such as `colr_name` or `legend_order` and, for `ggcorrhm()`, `high`, `mid`, `low` etc will be ignored to instead give the user full control. This allows for more flexibility, such as changing the breaks, bin sizes, or changing how out of bounds values are treated.
...
# Normal limits
plt1 <- gghm(plt_dat, colr_scale = "RdYlGn",
             cell_labels = TRUE)
...
```{r, fig.height=5, fig.width=10, fig.alt='Two correlation heatmaps with a gradient of values going from low values in the top left to high values in the bottom right. The scales are binned and the first one is binned into four bins with breaks at -1, -0.5, 0, 0.5, and 1, while the second one has five bins with breaks at -1, -0.6, -0.2, 0.2, 0.6, and 1.'}

# Make plot data between -1 and 1 (like correlation data)
plt_dat_cor <- matrix(
  sapply(seq(-1, 0, length.out = 10), function(x) {
    seq(x, x + 1, length.out = 10)
  }),
  nrow = 10
)

plt1 <- ggcorrhm(plt_dat_cor, cor_in = TRUE, bins = 5)
plt2 <- ggcorrhm(plt_dat_cor, cor_in = TRUE, bins = 5L)

...
# Normal limits
plt1 <- gghm(plt_dat, col_scale = "RdYlGn",
             cell_labels = TRUE)
...
Size scales can be customised as well. `ggcorrhm()` uses an absolute value tansform for the default size scale and hides the legend as it becomes inaccurate (as can be seen in the plot below). `scale_size_area` can be passed to `size_scale` to still scale with the absolute value and keep a meaningful legend, at the cost of not being able to set the lower limit for the sizes.

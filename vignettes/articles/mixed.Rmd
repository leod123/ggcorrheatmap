...
In mixed layouts the two triangles can use different scales for colours and sizes by using the `colr_scale` argument.
...
ggcorrhm(mtcars, layout = c("br", "tl"), mode = c("21", "hm"),
         ...
         # colr_name can also take two values
         colr_name = c("This name will be ignored", "Top left"))
```

Use the `guide` argument in the `ggplot2` scale functions to set the order or hide legends if a scale object is passed in the `colr_scale` argument.
...
ggcorrhm(mtcars, layout = c("tr", "bl"), mode = c("19", "text"),
         ...
         colr_name = c("Names that will", "be ignored"))
```
...
ggcorrhm(mtcars, layout = c("tl", "br"), mode = c("hm", "hm"),
         ...
         ), bins = 5, high = "pink", mid = "beige", low = "lightblue")
```

...
In mixed layouts the two triangles can use different scales for colours and sizes. Naturally, when one triangle uses a fill scale and the other a colour scale, using the `fill_scale` and `col_scale` arguments results in the two triangles using different scales.

```{r, fig.alt='mtcars correlation heatmap with normal cells in the top right and coloured text in the bottom left triangle. The cells in the top right triangle use the viridis "mako" scale while the text in the bottom left use the viridis "magma" scale. Each scale has a legend on the right (with the same name: "Pearson r").'}
ggcorrhm(mtcars, layout = c("tr", "bl"),
         fill_scale = scale_fill_viridis_c(option = "G"),
         col_scale = scale_colour_viridis_c(option = "A"),
         # Mixed layout with fill and colour scales hides one legend by default
         # Make both show up with show_legend
         show_legend = c(fill = TRUE, colour = TRUE))
```

When the two triangles use the same kind of scale (fill, colour), `fill_scale` and `col_scale` can take a list with two scales.
...
ggcorrhm(mtcars, layout = c("br", "tl"), mode = c("21", "hm"),
         ...
         # Fill name can also take two values
         fill_name = c("Bottom right", "Top left"))
```

Use the `guide` argument in the `ggplot2` scale functions to set the order or hide legends.
...
ggcorrhm(mtcars, layout = c("tr", "bl"), mode = c("19", "text"),
         ...
         col_name = c("First legend", "Second legend"))
```

If one of the scales is `NULL`, the default scale is used instead (and can be customised with the `high`, `mid`, `low` etc arguments).

```{r, fig.alt='mtcars correlation heatmap. The cells in the top left triangle use a divergent colour scale going from blue to white to red, while the bottom right cells use a binned scale with the Brewer "PrGn" palette, going from green to white to purple.'}

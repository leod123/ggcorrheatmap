...
```{r, fig.height=7, fig.width=8, fig.alt='A heatmap of mtcars with scaled columns. Rows and columns are clustered and each dimension has one continuous and one discrete annotation. The cells are coloured with the Viridis magma scale and are labelled by integers from 1 to 352.'}
...
When introducing gaps the `facet_rows` and `facet_cols` arguments allow for facet membership specification using columns in the input. It is still possible to specify the splits with the `split_rows` and `split_cols` arguments for the same behaviour as in `gghm()`.

```{r, fig.alt='Two mtcars heatmaps. The first one has two row splits and one column split with facet labels, and the rows and column are in a different order from the raw data. The second heatmap has one row and one column split with no facet labels.'}
plt1 <- mtcars_long %>%
...
```{r, fig.height=3, fig.width=7, fig.alt='A heatmap showing correlations between the columns in mtcars (in the columns) and the columns iris (in the rows, excluding the Species column).'}
...
```{r, fig.alt='A heatmap of the mtcars data with scaled columns. The cells are labelled with the cell values, rounded to integers.'}
...
```{r, fig.height=2.3, fig.width=7, fig.alt='A heatmap of the first 32 rows of the iris dataset, transposed to a 4x32 matrix. The cells are coloured with the Viridis magma colour scale and labelled with + at high values or - at low values, or nothing in-between.'}
iris_long %>%
...
```{r, fig.alt='A heatmap of a symmetric 15x15 matrix with values ranging between -1 and 1 in a gradient from the bottom left to the top right along the diagonal. The top left triangle uses the Viridis mako colour scale with cells labelled with +. The bottom right triangle uses the magma colours and has no labels. '}
df_long %>%
...
```{r, fig.alt='mtcars correlation matrix. The top right triangle is marked with correlation values. The bottom left triangle has no labels.'}
# `cor_in` is `TRUE`
mtcars_cor %>%
  # Add layout column
...
```{r, fig.alt='mtcars correlation matrix with diagonal running from the bottom left to the top right. The cells in the top left triangle are labelled with p-values and asterisks while the cells in the bottom right triangle are marked with correlation values.'}
...
```{r, fig.alt='mtcars correlation matrix with p-value labels with asterisks in both triangles. '}
mtcars_long %>%
...
Combine this with `add_mixed_layout` for more control in mixed layouts. Below is an example of showing unadjusted and adjusted p-values in different triangles of the heatmap.

```{r, fig.height=5, fig.width=10, fig.alt='Two mtcars correlation heatmaps. The first one has p-values in the cells. Only p-values below 0.05 are shown, and the bottom left triangle shows unadjusted p-values while the top right triangle shows adjusted p-values (being slightly higher). The second plot has swapped the p-values for asterisks in the bottom left triangle and pluses in the top right triangle.'}
plt_dat <- mtcars_long %>%
  # Add p-values
